...
import os, sys, collections, argparse, math
...
## Times-Roman it seems. This is currently hardcoded to my Fedora system.
rl_config.T1SearchPath = ["/usr/share/fonts/truetype/liberation/", "/usr/share/fonts/liberation/"]
...
def main(argv):
	...
	parser.add_argument("-f", "--file", action="store", dest="cleanupfile", help="path to cleanup result file", metavar="FILE")
	parser.add_argument("-o", "--output", action="store", dest="outputfile", help="path to output file", metavar="FILE")
	args = parser.parse_args()

	## path of the gzip compressed releases file
	if args.cleanupfile == None:
