...
depositores.append(re.compile('depósito legal'))
depositores.append(re.compile('deposito legal'))
depositores.append(re.compile('de?s?p*ós*i?r?tl?o?i?\s*l+e?g?al?\.?'))
depositores.append(re.compile('des?p?os+ito?\s+legt?al?\.?'))
depositores.append(re.compile('legal? des?posit'))
depositores.append(re.compile('dep\.\s*legal'))
depositores.append(re.compile('dip. legal'))
depositores.append(re.compile('dip. leg.'))
depositores.append(re.compile('dipòsit legal'))
depositores.append(re.compile('dipósit legal'))
depositores.append(re.compile('dep; legal'))

# then a slew of misspellings and variants
depositores.append(re.compile('deposito légal'))
depositores.append(re.compile('deposito legál'))
depositores.append(re.compile('depósito legl'))
depositores.append(re.compile('depósito lgeal'))
depositores.append(re.compile('depodito legal\.?'))
depositores.append(re.compile('depòsito? legal\.?'))
depositores.append(re.compile('déposito legal\.?'))
depositores.append(re.compile('depós?tio legal\.?'))
depositores.append(re.compile('dep\.?\s*legal\.?'))
depositores.append(re.compile('d\.?\s*legal\.?'))
depositores.append(re.compile('de?pto\.?\s*legal\.?'))
depositores.append(re.compile('depótiso legal'))
depositores.append(re.compile('depósitio legal'))
depositores.append(re.compile('depósiti legal'))
depositores.append(re.compile('deposrito legal'))
depositores.append(re.compile('deoósito legal'))
depositores.append(re.compile('depóaito legal'))
depositores.append(re.compile('depõsito legal'))
depositores.append(re.compile('depñosito legal'))
depositores.append(re.compile('deposiro legal\.?'))
depositores.append(re.compile('depósito légal'))
depositores.append(re.compile('déposito légal'))
depositores.append(re.compile('d\.\s*l\.'))
depositores.append(re.compile('dep\.\s*leg\.'))
depositores.append(re.compile('dep.\s*l.'))
depositores.append(re.compile('deposito lagal'))
depositores.append(re.compile('depósito lagal'))
depositores.append(re.compile('depósito degal'))
depositores.append(re.compile('depósito leagal'))
depositores.append(re.compile('depóosito legal'))
depositores.append(re.compile('depósite legal'))
depositores.append(re.compile('sepósito legal'))
depositores.append(re.compile('deopósito legal'))
depositores.append(re.compile('depásito legal'))
depositores.append(re.compile('depôsito legal'))
depositores.append(re.compile('depỏsito legal'))
depositores.append(re.compile('dep\'osito legal'))
depositores.append(re.compile('legal? des?posit'))
depositores.append(re.compile('legak des?posit'))
depositores.append(re.compile('legai des?posit'))
depositores.append(re.compile('legal depos?t'))
depositores.append(re.compile('legal dep\.'))

# basque DL:
# http://www.euskadi.eus/deposito-legal/web01-a2libzer/es/impresion.html
depositores.append(re.compile('l\.g\.'))

depositovalres = []
# deposito values, probably does not capture everything
depositovalres.append(re.compile('[abcjlmopstvz][\s\.\-/_:]\s*\d{0,2}\.?\d{2,3}\s*[\-\./_]\s*(?:19|20)?\d{2}'))
depositovalres.append(re.compile('(?:ab|al|as|av|ba|bi|bu|cc|ca|co|cr|cs|gc|gi|gr|gu|hu|le|lr|lu|ma|mu|na|or|pm|po|sa|se|sg|so|ss|s\.\s.|te|tf|t\.f\.|to|va|vi|za)[\s\.\-/_:]\s*\d{0,2}\.?\d{2,3}\s*[\-\./_]\s*(?:19|20)?\d{2}'))

# label code
#labelcodere = re.compile('\s*(?:lc)?\s*[\-/]?\s*\d{4,5}')
labelcodere = re.compile('\s*(?:lc)?\s*[\-/]?\s*\d{4,6}$')

masteringsidre = re.compile('\s*(?:ifpi)?\s*l\w{3,4}$')
mouldsidre = re.compile('\s*(?:ifpi)?\s*\w{4,5}$')
